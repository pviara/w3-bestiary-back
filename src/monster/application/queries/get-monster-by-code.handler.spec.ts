...
import { GetMonsterByCodeHandler, GetMonsterByCodeQuery } from './get-monster-by-code.handler';
...
describe('GetMonsterByCodeHandler', () => {
   ...
   describe('execute', () => {
       it('should call MonsterRepository getByCode method with query "code" and "lang" properties', async () => {
           // arrange
           const query = new GetMonsterByCodeQuery(
               'code',
               'lang'
           );
           ...
           // assert
           expect(monsterRepository.getByCode).toBeCalledWith(query.code, query.lang);
       });
   });
});